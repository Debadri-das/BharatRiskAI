...
def emergency(payload: EmergencyCreate, db: Client = Depends(get_db)):
    return create_emergency(db, payload)


@router.get("/emergencies")
def emergencies(db: Client = Depends(get_db)):
    return [emergency_payload(item) for item in list_emergencies(db)]


@router.post("/emergency/{emergency_id}/assign")
def assign(emergency_id: int, payload: AssignRequest, db: Client = Depends(get_db)):
    try:
