...
def citywide_nowcast(live: bool = Query(False, description="Fetch live weather from Open-Meteo"), db: Session = Depends(get_db)):
...
def zone_nowcast(zone_id: int, live: bool = Query(False), db: Session = Depends(get_db)):
...
def nowcast_alerts(live: bool = Query(False), db: Session = Depends(get_db)):
...
def simulate_nowcast_storm(payload: NowcastSimulationRequest, db: Session = Depends(get_db)):
    """Simulate severe weather progression under adjusted cloudburst, CAPE, and radar parameters."""
    query = db.query(Zone)
    if payload.zone_ids:
        query = query.filter(Zone.id.in_(payload.zone_ids))
    zones = query.all()
