...
export default function Header() {
  ...
  return <header className="topbar">
    ...
    <div className="topbar-meta"><span className={`feed-status ${isLive ? 'live' : 'stale'}`}><span className="status-dot" />{isLive ? 'LIVE RISK FEED' : 'LOCAL SNAPSHOT'}</span><span className="updated-label">{updatedLabel}</span><span className="network-status">{online ? (meshConnected ? <><Wifi size={16} /> Mesh linked</> : <><Wifi size={16} /> Online</>) : <><WifiOff size={16} /> Offline</>}</span><NotificationCenter /></div>
  </header>;
}
